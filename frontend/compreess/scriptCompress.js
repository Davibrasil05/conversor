document.addEventListener("DOMContentLoaded", () => {
    ...
    form.addEventListener('submit', async (event) => {
        ...
        try {
            const response = await fetch('http://localhost:3001/upload-compress', {
                method: 'POST',
                body: formData,
            });
            ...
            if (response.ok) {
                ...
                linkElement.innerText = 'Clique aqui para baixar imagem comprimida';
                linkElement.download = 'output.pdf';
                document.body.appendChild(linkElement);
                showAlert('imagem comprimida!', 'success');
            } else {
